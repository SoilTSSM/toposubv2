# ==================================================================== SETUP
# ==================================================================== INFO in =
# ele,asp,slp (minimum) out = listpoints.txt file, landforms.tif map
...
Nclust <- args[2]
Ngrid <- args[3]
# # test if there is at least one argument: if not, return an error if
# (length(args)==0) { # stop('At least one argument must be supplied (Nclust)',
# call.=FALSE)} print('WARNING: using default value Nclust=10') args[1] = 10}
# #else if (length(args)==1) { # default output file # args[1] = 10 #}

# ==================================================================== PARAMETERS
# FIXED ====================================================================
# nFuzMem=10 #number of members to retain
iter.max <- 50  # maximum number of iterations of clustering algorithm
nRand <- 1e+05  # sample size
...
# sample inputs need to generalise to accept 'data' argument
samp_dat <- sampleRandomGrid(nRand = nRand, predNames = predNames2)

# find number of clusters required if (findn==1){Nclust=findN(scaleDat=scaleDat,
# nMax=1000,thresh=0.05)}
...
# if(randomKmeansInit==T){ cbind(scaleDat_samp[5],scaleDat_samp[4],
# scaleDat_samp[3], scaleDat_samp[2], scaleDat_samp[1])->scaleDat_samp}

# remove NA's from dataset (not tolerated by kmeans)
scaleDat_samp2 <- na.omit(scaleDat_samp)
# kmeans on sample

# clust1=Kmeans(scaleDat=scaleDat_samp2,iter.max=iter.max,centers=Nclust,
# nstart=nstart1)
clust1 <- kmeans(x = scaleDat_samp2, centers = Nclust, iter.max = iter.max, nstart = nstart1, 
    trace = FALSE)
...
# clust2=Kmeans(scaleDat=scaleDat_all2,iter.max=iter.max,centers=clust1$centers,
# nstart=nstart2)
clust2 <- kmeans(x = scaleDat_all2, centers = clust1$centers, iter.max = iter.max, 
    nstart = nstart2, trace = FALSE)
# http://stackoverflow.com/questions/21382681/kmeans-quick-transfer-stage-steps-exceeded-maximum
...
if (samp_reduce == TRUE)
{
    clust3 <- sample_redist(pix = length(clust2$cluster), samples = Nclust, thresh_per = thresh_per, 
        clust_obj = clust2)  # be careful, samlple size not updated only clust2$cluster changed
} else
{
    clust3 <- clust2
}
...
rm(clust2)
gc()
# confused by these commented out lines gridmaps$clust <- clust3$cluster
# write.asciigrid(gridmaps['landform'],
# paste(spath,'/landform_',Nclust,'.tif',sep=''),na.value=-9999)

# make map of clusters

# new method to deal with NA values index of non NA index
...
# writeRaster(raster(gridmaps['landform']),
# paste(spath,'/landform_',Nclust,'.tif',sep=''),NAflag=-9999,overwrite=T)
...
# remove this unecessary (?) I/O write to disk for cmeans(replaced by kmeans 2)
# write.table(samp_mean,paste(spath, '/samp_mean.txt' ,sep=''), sep=',',
# row.names=FALSE) write.table(samp_sd,paste(spath, '/samp_sd.txt' ,sep=''),
# sep=',', row.names=FALSE)

# make driving topo data file lsp <- listpointsMake(samp_mean=samp_mean,
# samp_sum=samp_sum)
...
# make horizon files MOVED TO SEPERATE SCRISPT hor(listPath='.')
...
# write.table(zoneStats, paste(spath,'/landcoverZones.txt',sep=''),sep=',',
# row.names=F)
