# solar functions add options eg terrain effects
...
solarCompute <- function(swin, toa, dates, mf, tz)
{
    ...
    # very slow need to vectorise kdvec=c() for (i in 1:length(swPoint)){ y=kt[i]
    # kd=0.952 - 1.041 * exp(1) ^ - exp(2.300-4.702*y) kdvec=c(kdvec, kd) }
    y <- kt
    kdvec <- 0.952 - 1.041 * exp(1)^-exp(2.3 - 4.702 * y)
