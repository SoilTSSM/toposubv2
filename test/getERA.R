...
# register set up $HOME/.ecmwfapirc
...
# GET BBOX FROM MF
# ===============================================================================
# tol=0.1 n=max(mf$lat+tol) s=min(mf$lat-tol) e=max(mf$lon+tol) w=min(mf$lon-tol)
...
for (i in 1:length(parNameSurf))
{
    ...
    getERA(dd = dd, t = t, grd = grd, stp = stp, lt = lt, typ = typ, par = par, ar = ar, 
        tar = tar, plev = plev, workd = eraDir)
}
...
# for(i in 1:length(grb)){ system(paste('rm ',workd,'/',grb[i],sep='')) }
