...
sampleRandomGrid <- function(nRand, predNames)
{
    ...
    x <- sampleRandom(ele, nRand, cells = T, na.rm = T)
    rand_vec <- x[, 1]
    ...
    {
        sample_vec <- extract(stk[[i]], rand_vec)
...
# @celenius center=true just means remove the mean, and scale=TRUE stands for
# divide by SD; in other words, with both options active, you're getting
# standardized variables (with mean 0, unit variance, and values expressed in SD
# units)
...
simpleScale <- function(data, pnames)
{
    ...
    for (pred in pnames)
    {
        b <- as.vector(assign(paste(pred, "W", sep = ""), scale(data[pred], center = TRUE, 
            scale = TRUE)))
...
findN <- function(scaleDat, nMax, thresh)
{
    ...
    # calc. approx number of cluster - dont need to do for weighted run make exp
    # sequence
    nseq <- c()
    ...
    for (i in nseq)
    {
        f[i] <- (kmeans(scaleDat, centers = i)$betweenss)/(kmeans(scaleDat, centers = i)$tot.withinss)
    }
    # na.omit(wss)->wss wss[1:length(wss)]->wss
    f <- na.omit(f)
    f <- f[1:length(f)]
    nseq <- c(1, nseq)
...
Kmeans <- function(scaleDat, iter.max, centers, nstart)
{
    ...
    # make map of clusters gridmaps$clust <- clust$cluster gridmaps$landform <-
    # as.factor(clust$cluster)
    return(clust)
}
...
sample_redist <- function(pix, samples, thresh_per, clust_obj)
{
    ...
    dist_matrix  # calculate dists
 <- dist(clust_obj$centers, 
        method = "euclidean")
    dm <- as.matrix(dist_matrix)
    
    samples_dead  #  define threshold
 <- (pix/samples) * thresh_per
 ...
 <- sort(clust_obj$size)
    v  #***double check***
 <- data.frame(rank_size, sort_size)
    samples_rm_vec  # vector of sample index to be removed
 <- v$rank_size[v$sort_size < 
        samples_dead]
    
    
    for (i in samples_rm_vec)
    {
        # loop thru samples_rm
        sort_dm <- sort(dm[i, ], decreasing = T)
        a <- sort_dm[1]
        cluster_redist  # nearest neighbour cluster to redistribute pixels to 
 <- as.numeric(names(a))
...
# kmClust<-function(gridmaps,Nclust, nMax=170,thresh=0.05, nstart=5, esPath,
# WSS=0){
...
hor <- function(listPath)
{
    ...
    listpoints <- read.table(paste(listPath, "/listpoints.txt", sep = ""), header = T, 
        sep = ",")
    ID <- listpoints$id
    ...
    hor.el <- (((acos(sqrt(svf)) * 180)/pi) * 2) - slp
    hor.el <<- round(hor.el, 2)
    ...
    for (hor in hor.el)
    {
        ...
        write.table(hor, paste(listPath, "/hor/hor_point", IDn, ".txt", sep = ""), 
...
linMod <- function(param, col, predNames, mod = "gls", scaleIn = FALSE)
{
    ...
    # read in listpoints
    listpoints <- read.table(paste(esPath, "/listpoints.txt", sep = ""), sep = ",", 
        header = T)
    ...
    # pc.dem <- prcomp(x=listpoints[predNames]) demdata <- as.data.frame(pc.dem$x)
    
    # fit <- lm(dat$meanX ~ demdata$PC1+ demdata$PC2 + demdata$PC3) summary(fit) #
    # show results coefficients(fit)->coef round(summary(fit)$r.squared,2)->r2
    # coef[1:length(predNames)+1]->x coeffs<-c(col,x,r2)
    
    # scale inputs
    if (scaleIn == TRUE)
    {
        dat <- simpleScale(dat, pnames = c("meanX", predNames))
    }
    
    # select ols or gls
    if (mod == "ols")
    {
        fit <- lm(dat$meanX ~ dat$ele + dat$slp + dat$svf + dat$aspC + dat$aspS)
    } else
    {
    ...
    # !remember order of coeffs is tied to order of header in cryosub_prog [order of
    # predNames]! ==> potential bug - fix
    summary(fit)  # show results
    coef <- coefficients(fit)
    r2 <- round(summary(fit)$r.squared, 2)
    x <- coef[1:length(predNames) + 1]
    coeffs <- c(col, x, r2)
...
fuzzyMember <- function(esPath, ext, cells, predNames, data, samp_mean, samp_sd, 
    Nclust)
    {
    ...
    for (c in (1:Nclust))
    {
        ...
        distmaps[[c]] <- data.frame(ele = tmp, slp = tmp, aspC = tmp, aspS = tmp, 
            svf = tmp)
        
        for (j in predNames)
        {
            distmaps[[c]][j] <- (((gridmaps@data[j] - samp_mean[c, j])/samp_sd[c, 
                j])^2)
            # distmaps[[c]][j] <- (((aspC-class.c[c,j])/class.sd[c,j])^2)
        }
        v <- sqrt(rowSums(distmaps[[c]], na.rm = T, dims = 1))
        n <- setValues(ext, v)
...
topMembers <- function(esPath = esPath, Nclust = Nclust, nFuzMem)
{
    ...
    # whichmax=function(x){a<-which.max(x);return(a)} a=stackApply(rst,
    # indices=rep(1,100),fun=max,na.rm=T) which.max2 <- function(x, ...) which.max(x)
    # wsa <- stackApply(rst, rep(1,117), fun=which.max2, na.rm=NULL)
    
    sortStack_index <- function(x, i)
    {
        a <- order(x, decreasing = T)[i]
        return(a)
    }
    sortStack_val <- function(x, i)
    {
        a <- sort(x, decreasing = T)[i]
        return(a)
    }
    
    # x=c(2,4,6,2,9,1,4,6) wsa <- stackApply(rst, rep(1,100), fun=sortStack, i=1) wsa
    # <- stackApply(rst, rep(1,100), fun=sortStack, i=1)
    ...
    # r=raster(memberIndex[,,1]) r=t(r) extent(r)<-extent(wsa) r=flip(r,
    # direction='y')
}
...
timeSeriesCut <- function(esPath, col, sim_dat, beg, end)
{
    ...
    # Period: beg <- '01/07/2010 00:00:00' end <- '01/07/2011 00:00:00'
    ...
    tmpsCut <- tmps[lines, ]
    nlines <- sum(lines)
    sim_dat_cut <- as.data.frame(tmpsCut)
    return(sim_dat_cut)
}
# ==============================================================================
...
# ==============================================================================
# box8 - neighbours = 2,7,9,14 spatialise sample 6 box 8

# samp=6 mbox=8 nbrs=c(2,9,14,7) #clockwise from north predNames<-c('ele', 'slp',
# 'asp', 'svf') wm=0.7 #weight main sample wn=0.3 # weight of weighted 4
# neighbour samples in nspace

spatialWeight <- function(mbox, samp, nbrs, predNames = c("ele", "slp", "asp", "svf"), 
    wm = 0.7, wn = 0.4)
    {
    m <- read.table(paste("/home/joel/experiments/alpsSim/box", mbox, "/listpoints.txt", 
        sep = ""), sep = ",", header = T)
    n1 <- read.table(paste("/home/joel/experiments/alpsSim/box", nbrs[1], "/listpoints.txt", 
        sep = ""), sep = ",", header = T)
    n2 <- read.table(paste("/home/joel/experiments/alpsSim/box", nbrs[2], "/listpoints.txt", 
        sep = ""), sep = ",", header = T)
    n3 <- read.table(paste("/home/joel/experiments/alpsSim/box", nbrs[3], "/listpoints.txt", 
        sep = ""), sep = ",", header = T)
    n4 <- read.table(paste("/home/joel/experiments/alpsSim/box", nbrs[4], "/listpoints.txt", 
        sep = ""), sep = ",", header = T)
    ...
    # read in main result
    datin <- read.table(paste("/home/joel/experiments/alpsSim/box", mbox, "/meanX_X100.000000.txt", 
        sep = ""), sep = ",", header = F)
    datm <- datin[samp, ]
    ...
    for (i in 1:length(box))
    {
        ...
        datin <- read.table(paste("/home/joel/experiments/alpsSim/box", n, "/meanX_X100.000000.txt", 
            sep = ""), sep = ",", header = F)
        dat <- datin[sample[i], ]
        datvec <- c(datvec, dat)
    }
...
spatial <- function(col = col, esPath = esPath, format = ".tiff", Nclust = Nclust)
{
    ...
    writeRaster(fuzRst, paste(esPath, "/result/out/fuz_", col, ".tif", sep = ""), 
        NAflag = -9999, overwrite = T)
    # endScript
}

fuzSpatial <- function(col = col, esPath = esPath, format = ".tiff", Nclust = Nclust, 
    mask)
    {
    ...
    writeRaster(fuzRstMask, paste(esPath, "/result/out/fuz_", col, format, sep = ""), 
        NAflag = -9999, overwrite = T)
    
    # endScript
}

### speed up sum(rasterstack) operation by spitting raster into blocks of 10 sum
### each then sum result of each result.  only multiples of 10 allowed

fuzSpatial_subsum <- function(col = col, esPath = esPath, format = ".tif", Nclust = Nclust, 
    mask)
    {
    rstdir <- paste(esPath, "/raster_tmp", sep = "")
    ...
    for (i in 1:(Nclust/10))
    {
        ...
        writeRaster(rst, paste(rstdir, "/fuzblock_", i, format, sep = ""), NAflag = -9999, 
            overwrite = T)
        print(i)
    }
    t2 <- Sys.time() - t1
    ...
    writeRaster(fuzRstMask2, paste(esPath, "/fuz_", col, format, sep = ""), NAflag = -9999, 
        overwrite = T)
...
crispSpatial_noinform <- function(col, Nclust, esPath, landform)
{
    ...
    meanX <- as.vector(meanX$x)
    l <- length(meanX)
    seq <- seq(1, l, 1)
    seq <- as.vector(seq)
    
    meanXdf <- data.frame(seq, meanX)
    
    
    rst <- subs(land, meanXdf, by = 1, which = 2)
    
    writeRaster(rst, paste(esPath, "/crispRst_noinform/", col, "_", l, ".asc", sep = ""), 
        overwrite = T)
...
plotRst <- function(esPath, rootRst, name, formt = ".asc", Nclust)
{
    ...
    if (rootRst == "crisp")
    {
        rootRst <- paste(esPath, "/crispRst", sep = "")
    }
    if (rootRst == "fuzzy")
    {
        rootRst <- paste(esPath, "/fuzRst", sep = "")
    }
...
{
    expSeq <- seq(minExp, maxExp, 1)
    expSeq <- formatC(expSeq, width = 6, digits = 0, flag = "0", format = "f")
    for (i in expSeq)
    {
        
        file.copy(paste(masterRoot, "/", filename, sep = ""), paste(expRoot, i, sep = ""), 
            overwrite = TRUE)
...
# validate points Requirements: spath='/home/joel/src/hobbes/results4/b8'
# demLoc='/home/joel/src/hobbes/results/b10/box10/preds/ele.tif'
# dat=read.table('/home/joel/data/PERMOS/sites_completeMeta.txt', sep=',',
# header=T) names(dat)[4]<-'ele' names(dat)[7]<-'slp' names(dat)[8]<-'asp'
# names(dat)[9]<-'svf' dat$aspC<-cos(dat$asp*(pi/180))
# dat$aspS<-sin(dat$asp*(pi/180)) predNames<-c('ele', 'slp', 'svf', 'aspC',
# 'aspS') Nclust=100 esPath=spath data<-dat fuzzy.e=1.4

## read in sample centroid data samp_mean <- read.table(paste(spath,
## '/samp_mean.txt' ,sep=''), sep=',',header=T) samp_sd <- read.table(paste(spath,
## '/samp_sd.txt' ,sep=''), sep=',', header=T)


## ================== COMPUTE FUZZY MEMBERSHIP MATRIX [SAMPLES * POINTS]
## =============================
valPoints <- function(esPath, cells, predNames, data, samp_mean, samp_sd, Nclust, 
    fuzzy.e)
    {
    ...
    for (c in (1:Nclust))
    {
        ...
        n  #v
 <- sqrt(rowSums(distmaps[[c]], na.rm = T, dims = 1))
...
calcFuzPoint <- function(dat, fuzMemMat)
{
    ...
    # dat=read.table('/home/joel/src/hobbes/results/b10/box10/meanX_X100.000000.txt',
    # sep=',', header=F)
