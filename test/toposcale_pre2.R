...
# ==================================================================== PARAMETERS
# FIXED ====================================================================
# PRESSURE LEVEL FIELDS (AND DEWT) infileG='gpot.nc' infileT='tpl.nc'
# infileRh='rhpl.nc' infileU='upl.nc' infileV='vpl.nc' infileD='dt.nc'
infilePLEVEL <- "PLEVEL.nc"

# SURFACE FIELDS lwFile='strd.nc' swFile='ssrd.nc' pFile='p.nc' tFile='t.nc'
# toaFile='toa.nc'
infileSURF <- "SURF.nc"
...
######################################################################################################## TOPOSCALE PREPROCESS DATA interpolate to 3h timestep includes dewT (surface
######################################################################################################## field) which comes at 6h
...
# write.table(dates,paste(outRootPl, '/dates', sep=''), sep=',', row.names=F,
# col.names=F)
...
# convert accumulated values to timestep averages precip units changed 3d to 2d
# matrix
...
######################################################################################################## COMPUTE SURFACE RELATIVE HUMIDITY (for LWin computation) some newer docs
...
######################################################################################################## CLEAN UP system('rm -r /home/joel/data/tscale/tmp/pressurelevel/') system('rm
######################################################################################################## -r /home/joel/data/tscale/tmp/surface')
