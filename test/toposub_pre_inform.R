...
samp_reduce <- FALSE
# ==================================================================== TOPOSUB
# PREPROCESSOR INFORMED SAMPLING
# ====================================================================
setwd(gridpath)

# set tmp directory for raster package setOptions(tmpdir=paste(gridpath, '/tmp',
# sep=''))
...
x <- cbind("tv", pvec, "r2")
write.table(x, paste(gridpath, "/coeffs.txt", sep = ""), sep = ",", col.names = F, 
    row.names = F)
write.table(x, paste(gridpath, "/decompR.txt", sep = ""), sep = ",", col.names = F, 
    row.names = F)
...
coeffs <- linMod2(meanX = meanX, listpoints = listpoints, predNames = predNames2, 
    col = targV, svfCompute = FALSE)  #linear model

write(coeffs, paste(gridpath, "/coeffs.txt", sep = ""), ncolumns = 7, append = TRUE, 
    sep = ",")  # 6 cols if no svf
...
# y<-rbind(predNames) y <- cbind(y,'r2') write.table(y,
# paste(egridpath,'/coeffs_Mean.txt',sep=''), sep=',',col.names=F, row.names=F)
# write(coeffs_vec,
# paste(egridpath,'/coeffs_Mean.txt',sep=''),ncolumns=(length(predNames)+1),
# append=TRUE, sep=',')
# weightsMean<-read.table(paste(egridpath,'/coeffs_Mean.txt',sep=''),
# sep=',',header=T)
...
# kmeans on sample
clust1 <- Kmeans(scaleDat = informScaleDat_samp, iter.max = iter.max, centers = Nclust, 
    nstart = nstart1)
# scale whole dataset
...
clust2 <- Kmeans(scaleDat = informScaleDat_all, iter.max = iter.max, centers = clust1$centers, 
    nstart = nstart2)
...
# paste(egridpath,'/landform_',Nclust,'.tif',sep=''),na.value=-9999)

# make map of clusters

# new method to deal with NA values vector of non NA index
