# TODO: Add comment Author: stgruber
...
# require('gregmisc') - depracated does it really need these packages?
# require(gdata) require(gtools) require(gmodels)
...
gt.par.fline <- function(fs, keyword)
{
    ...
    for (l in 1:nl)
    {
        ...
        if ((substr(line, 1, 1) != comchar) * (nchar(line) >= 1))
        {
            ...
            if ((toupper(varname) == keyword)) 
                found <- TRUE
        }
        if (found == TRUE) 
            break  #exit loop if keyword was found
    }
...
{
    # fs --> file string containing entire parameter file ln --> line number to read
...
gt.par.wline <- function(fs, ln, vs)
{
    ...
    if (length(vs) > 1) 
        vs <- paste(vs, sep = "", collapse = vecchar)
    line <- trim(fs[ln])  #assign line, but first remove leading/trailing blanks
...
gt.par.wvar <- function(fs, keyword, vs, type = "NUMERIC")
{
    ...
    {
        # if line does not exist, append at end
...
{
    # DIRECTORY STRUCTURE experiment_root/ --> base directory of an experiment
    # experiment_root/_master/ --> simulation template, experiments use common data
    # from here experiment_root/_control/ --> executable, source, batch.txt,
    # documentation experiment_root/000001/ --> path for experiment run 1 fs -->
    # filestring with modified parameter file to define experiment
...
gt.exp.write <- function(eroot_loc, eroot_sim, enumber, fs)
{
    ...
    # experiment_root/_master/ --> simulation template, experiments use common data
    # from here experiment_root/_control/ --> executable, source, batch.txt,
    # documentation experiment_root/000001/ --> path for experiment 1 fs -->
    # filestring with modified parameter file to define experiment eroot_loc -->
    # experiment root on local machine (where simulation is prepared) eroot_sim -->
    # experiment root on simulation machine (used for batch file writing)
    comchar <- "!"  #indicate comments
    parfilename <- "geotop.inpts"  #standard name of parameter file
    # make directories ----------------------------------
...
# ==============================================================================
# This script will make a list of experiment directories contained in a local
# simmulation directory (eroot_loc) and write bach files for the Schroedinger
# grid engine.  Execute on Schroedinger by typing 'qsub myscript.sh' (or any
# other filename) You can control the state of your job by 'qstat' VARIABLES - -
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - eroot_loc:
# experiment root directory on local machine (e.g., /home/stgruber/exp01)
# eroot_sim: experiment root directory on productive (Schroedinger) file system
# enumber_vec: vector of experiment numbers (e.g., 1:60) name: short name of job
# to be displayed in Schroedinger SGE statistics (e.g., EXP01) maxtime: maximum
# time in seconds needed for GEOtop run (see below) SCHROEDINGER QUEUEING SYSTEM
# - - - - - - - - - - - - - - - - - - - - - - - - ----name---- ---maxtime--
# very-short.q 1800 short.q 86400 med.q 172800 long.q 259200
# ==============================================================================
gt.exp.batch <- function(eroot_loc, eroot_sim, enumber_vec, name, maxtime, executable)
{
    ...
    fs <- c(fs, paste("  gtcop=`sed -n ", "\"", "${counter_cop}p", "\"", " $eroot_sch/_control/batch_schroedinger.txt` #get line from job list", 
        sep = ""))
    fs <- c(fs, paste("  gtjob=`sed -n ", "\"", "${counter_run}p", "\"", " $eroot_sch/_control/batch_schroedinger.txt` #get line from job list", 
        sep = ""))
    # copy and run job
    ...
    {
        epath <- eroot_sim & "/" & formatC(round(enumber_vec[enumber], 0), width = 6, 
            flag = "0") & "/"
    ...
    for (enumber in enumber_vec)
    {
        epath_in <- eroot_sim & "/" & formatC(round(enumber, 0), width = 6, flag = "0") & 
            "/in"
...
# MAKE HORIZON FILES
# ==============================================================================
# Needs to be updated make series of standard horizon files - e.g., hor0001.txt
# for 10¡
...
# ==============================================================================
# Changes: 26/03/2012: removed 'eroot_loc' as firs parameter [Stephan Gruber]
points.make <- function(hor_seq, ele_seq, slp_seq, asp_seq, lan_seq, soi_seq, dis_seq, 
    fre_seq, swe_seq, cur_seq)
    {
    # initialize names
    ID_vec <- NULL
    ele_vec <- NULL
    ...
    topo <- data.frame(ID = ID_vec, ele = ele_vec, slp = slp_vec, asp = asp_vec, 
        sky = sky_vec, landcover = lan_vec, soil = soi_vec, dist = dis_vec, free = fre_vec, 
        maxswe = swe_vec, curvNS = cur_vec, curvWE = cur_vec, curvNwSe = cur_vec, 
        curvNeSw = cur_vec, hor = hor_vec)
    return(topo)
}
# ==============================================================================
...
# ==============================================================================
# dzmin: minimal z-spacing ('how fine is the grid') zmax: depth of lowermost node
# center ('how large is the domain') base: resolution reduction ('how strong is
# the grid corsened with depth')
...
# ==============================================================================
# For new parameterization, GEOtop 1.222 and higher nbot: number of layers with
# limited thickness near ground surface ntop: number of layers with limited
# thickness at top of snow pack nthick: number of layers in the middle that can
# have any thickness
gt.snow.discretize <- function(nbot, ntop, nthick)
{
    ...
    # return data frame
    return(data.frame(MaxSnowLayerNumber = round(ml, 0), ThickerSnowLayers = round(tl, 
        0)))
}
# ==============================================================================
...
# comment<-gt.rmatrix('/group/geotop/sometestfile.txt',comment=TRUE) usage to
# read data: comment<-gt.rmatrix('/group/geotop/sometestfile.txt')
gt.rmatrix <- function(infile, comment = FALSE)
{
    ...
    } else
    {
        ...
        # if first column is a date, then convert data format always looks like this:
        # 31/01/2004 13:24:00 1234567890123456789
        isDate <- TRUE
        data[, 1] <- as.character(data[, 1])
        if (nchar(data[1, 1]) != 16) 
            isDate <- FALSE
        if (substr(data[1, 1], 3, 3) != "/") 
            isDate <- FALSE
        if (substr(data[1, 1], 6, 6) != "/") 
            isDate <- FALSE
        if (substr(data[1, 1], 11, 11) != " ") 
            isDate <- FALSE
        if (substr(data[1, 1], 14, 14) != ":") 
            isDate <- FALSE
        if (isDate == TRUE) 
            data$Date <- as.POSIXct(data[, 1], GTtime, tz = "")
        return(data)
    }
}
# ==============================================================================
...
gt.wmatrix <- function(data, outfile, comment = "")
{
    ...
    write.table(data, file = outfile, append = FALSE, quote = FALSE, sep = ",", eol = "\n", 
...
# ==============================================================================
# INPUT RESTRICTIONS: Both data frames have a column called 'Date' in POSIXct and
# the column with precipitation is called 'Prec' RATIONALE AND METHOD: When only
# daily preciptation is available, this needs to be distributed to a
# higher-resolution (usually hourly) data set of other meteorologicl
# observations. In the absence of better models that may account for e.g.,
# convective clouding, precipitation is distributed randomly to avoid a bias with
# temperature affecting rain/snow proportions.  The variable 'hourly_max' limits
# the hourly precipitation. More than that is distributed to moew than one hour.
# ATTENTION: Daily precipitation is limited to hourly_max*24 !!
gt.input.prec.day2hour <- function(df.daily.prec, df.hourly.met, hourly_max)
{
    ...
    out <- merge(df.hourly.met, df.daily.prec, by = "Date", all.x = TRUE, incomparables = 0)
...
gt.run.combined <- function(eroot, ie, ListResultFiles, ListNamePrefix)
{
    ...
    if (nfiles > 1)
    {
        for (ifile in 2:nfiles)
        {
            ...
            AllRaw <- merge(AllRaw, new, by = c("Date", "Simulation_Period", "Run", 
                "IDpoint"))
        }
    }
...
# ==============================================================================
# This function creates a parameter set based on the data frame param.  Local or
# global parameter set generation can be chosen. Local means to keep all
# parameters to their default value and to only change one parameter at a time.
# Global means that the full parameter space is explored. If n1, n2... are the
# numbers of values per parameter, then the total number of experiments is for
# local (n1+n2+....*nN) and for global (n1*n2*....*nN).
gt.parameter_set.create <- function(param, method = local)
{
    ...
    if (npar > 1)
    {
        for (pn in 2:npar)
        {
            newcol <- data.frame(newcol = get(names(param)[pn], param)$default[1])
    ...
    if (toupper(method) == "LOCAL") 
        {
            parse <- TRUE
            # loop over parameters and fill in non-default columns loop over variables
            for (pn in 1:npar)
            {
                vec <- get(names(param)[pn], param)$values
                vec <- vec[vec != get(names(param)[pn], param)$default[1]]
                for (pv in 1:length(vec))
                {
                  # loop over values
                  newrow <- parr[1, ]
                  newrow[pn] <- vec[pv]
...
gt.parameter_set.extend <- function(parr, param)
{
    ...
    for (pn in 1:npar)
    {
        newcol <- data.frame(newcol = rep(get(names(param)[pn], param)$default[1], 
            nrows))
...
# ==============================================================================
# -read parameter file, make parameter data frame -loop over experiment
# directories, based on data frame, possibly restricted range -read listpoint
# file, combine with parameter data frame -read point output files, -loop over
# years and point number -parse to processor function to compute results -append
# to data frame -write output data frame in experiment directory ListResultFiles:
# c('soilT.txt','soilL.txt')
gt.pointout.batch <- function(eroot, parfile, topofile, exp_range, ListResultFiles, 
    ListNamePrefix, StartDate, function_processing)
    {
...
# ==============================================================================
# -read parameter file, make parameter data frame -loop over experiment
# directories, based on data frame, possibly restricted range -read listpo
gt.exp.subset.write <- function(eroot_loc, eroot_sim, enumber, fs, topo, ntopo_opt)
{
    ...
    # loop though topo list loop over number of topography setups write experiment
    for (tn in seq_along(1:ntopo))
    {
        ...
        if (tn != ntopo)
        {
            write.csv(topo[(tn - 1) * ntopo_opt + (1:ntopo_opt), ], file = file.path(paste(epath, 
                "/in", sep = ""), "listpoints.txt"), quote = FALSE, row.names = FALSE)
        }
        if (tn == ntopo)
        {
            write.csv(topo[(tn - 1) * ntopo_opt + (1:topo_last), ], file = file.path(paste(epath, 
                "/in", sep = ""), "listpoints.txt"), quote = FALSE, row.names = FALSE)
        }
