...
# ======================================================================== CALC
# WIND SPEED AND DIRECION FROM U/V
# ========================================================================
u <- uPoint
v <- vPoint
wdPoint <- windDir(u, v)
...
for (i in 1:npoints)
{
    ...
    SW <- round(sol[, i], 2)
    # sdir=round(solDir[,i],2) sdif=round(solDif[,i],2)
    ...
    {
        print(paste0("WARNING:", length(which(is.na(meteo) == TRUE)), "NANs found in meteofile: ", 
            simindex))
    }
    
    if (length(which(is.na(meteo) == TRUE)) == 0)
    {
        print(paste0(length(which(is.na(meteo) == TRUE)), "NANs found in meteofile: ", 
            simindex))
    }
    
    write.table(meteo, paste(simindex, "/meteo0001.txt", sep = ""), sep = ",", row.names = F, 
        quote = FALSE)
    ...
    write.table(listp, paste0(simindex, "/listpoints.txt", sep = ""), sep = ",", 
        row.names = F)
