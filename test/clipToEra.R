...
t1 <- Sys.time()
getERA(dd = dd, t = t, grd = grd, stp = stp, lt = lt, typ = typ, par = par, ar = ar, 
    tar = tar, plev = NULL, workd = wd)
t2 <- Sys.time()
t3 <- t2 - t1
...
# dem accuracy of these two extents is around half DEm pixel = 15m ie can be 15m
# difference in boudaries
...
plot(extent(eraExtent), col = "green", lwd = 2, main = "New extent of ERA-grids overlaid input DEM.", 
    sub = "New DEM outline (blue). Original ERA request (green)")
