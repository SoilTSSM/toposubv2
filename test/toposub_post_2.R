...
for (i in 1:Nclust)
{
    ...
    # cut timeseries
    sim_dat_cut <- timeSeriesCut(esPath = simindex, col = targV, sim_dat = sim_dat, 
        beg = beg, end = end)
    # mean annual values
...
if (fuzzy == TRUE)
{
    ...
    # fuzSpatial(col=targV, esPath=gridpath, format=predFormat,
    # Nclust=Nclust,mask=mask)
    fuzSpatial_subsum(col = targV, esPath = gridpath, format = predFormat, Nclust = Nclust, 
        mask = mask)
}
...
# writeRaster(rmerge, paste(epath,'rmerge3.tif',sep=''),
# NAflag=-9999,overwrite=T,options='COMPRESS=LZW') t4=Sys.time()-t1

# png('/home/joel/Documents/posters_presentations/kolloqium/alps.png',width=1200,height=900)
# plot(rmerge, col=matlab.like(100),maxpixels=3000000) dev.off()
