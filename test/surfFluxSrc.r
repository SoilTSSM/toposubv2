# surface flux functions source file 13/6/2012


############################################################################################# prepare IMIS data

# ============================================ make long/lat shape file of all
# IMIS points ============================================
...
# ============================================ make long/lat shape file for
# generic points ============================================ options
# project=longlat, ch1903+, ch1903
makePointShapeGeneric <- function(lon, lat, data, proj = "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")
{
    ...
    # p2 ='+proj=somerc +lat_0=46.95240555555556 +lon_0=7.439583333333333
    # +x_0=2600000 +y_0=1200000 +ellps=bessel +towgs84=674.374,15.056,405.346,0,0,0,0
    # +units=m +no_defs'
    
    # p3 ='+proj=somerc +lat_0=46.95240555555556 +lon_0=7.439583333333333 +k_0=1
    # +x_0=600000 +y_0=200000 +ellps=bessel +towgs84=674.374,15.056,405.346,0,0,0,0
    # +units=m +no_defs'
    
    # if (project=='longlat'){projection<-p1} if (project=='ch1903+'){projection<-p2}
...
# =================================================== returns dataframe of imis
# ID and gridbox membership ===================================================
getPointGridbox <- function(gridbox_raster = "~/experiments/bigsim/sim/_master/extent_sml.asc", 
    shp)
    {
    library(raster)
    library(rgdal)
    if (unlist(class(gridbox_raster))[1] == "RasterLayer")
    {
        gridBox <- gridbox_raster
    } else
    {
        gridBox <- raster(gridbox_raster)
    }
...
# ===================================================== get valid imis data (with
# data ie remove wind station) returns dataframe of
#-station ids
#-number datapoints
...
imisWithData <- function(var, file, startDate, endDate, origin, NAval = 9.97e+36)
{
    ...
    snow <- get.met.data(stationNr = 1, varName = var, startTime = startDate, endTime = endDate, 
        fileName = file, origin = origin)
    snow$data[snow$data > 1e+16] <- NA
    
    snodfAll <- data.frame(snow$data)
    for (stationNr in 1:stations)
    {
        print(stationNr)
        snow <- get.met.data(stationNr = stationNr, varName = var, startTime = startDate, 
            endTime = endDate, fileName = file, origin = origin)
        snow$data[snow$data > 1e+16] <- NA
        snodfAll <- data.frame(snodfAll, snow$data)
    }
    snodfAll <- snodfAll[2:(stations + 1)]
    ...
    for (i in 1:stations)
    {
        # if(colMeans(na.omit(snodfAll[i]),na.rm=T)<1000000 &
        # is.na(mean((snodfAll[i]),na.rm=T))==F)
        if (is.na(mean((snodfAll[i]), na.rm = T)) == F)
        {
            print(i)
            snodfSub <- data.frame(snodfSub, snodfAll[i])
        }
    }
    
    if (length(snodfSub) > 1)
    {
        snodfSub <- snodfSub[2:length(snodfSub)]
        
        stationNames <- names(snodfSub)
        n <- strsplit(stationNames, "X")
        x <- as.numeric(unlist(n, 2))
        
        station_vec  # removes '' strings from ctor
 <- x[is.na(x) == 
            F]
...
# ==================================================================== remove
# stations with data timeseries less than GT [%] complete
# ====================================================================
...
# ======================================================= get ele diff between
# grid and validation data station
# ========================================================
caLcEleDiff <- function(station, nbox, file = "/home/joel/data/imis/data/imis_data.nc")
{
    ...
    simDir <- paste("~/experiments/bigsim/sim/", formatC(nbox, width = 6, flag = "0"), 
        sep = "")
...
# 24/5/2012 DESCRIPTION: using ERA pressure level data, interpolates temperature
# profile to provide AirT timeseries for any given point in ERA grid cell domain
# IN: 1. ERA air temperature pressure level data (K) 2. ERA geopotential file
# (can be the same file) [m^2 s^-2] 3. height (m asl) of interpolation point 4.
# ERA grid box returns tair at stationHeight (a) interpolated to 3hr step or (b)
# aggregated to daily NEED TO MAKE ACCEPT NPRESSURE LEVELS ARGUMENT
eraAirTPreprocess <- function(nbox, map = NULL, var, lm, k1 = 1, k2 = 1460, stationHeight, 
    Tfile, Gfile, origin, interpolate, daily)
    {
    ...
    for (i in 1:(k2 - k1 + 1))
    {
        ...
        if (lm == FALSE)
        {
            t <- approx(gph_profile[i, ], t_profile[i, ], xout = stationHeight)[2]
            t <- t$y
        }
    ...
    if (daily == T)
    {
        t_vec <- aggregate.ts(t_vec, nfrequency = 0.25, FUN = mean)
    }
...
getSurfaceDataAirT <- function(file, aggregate)
{
    ...
    if (aggregate == T)
    {
        day <- as.POSIXct(strptime(format(met$Date, format = "%d/%m/%Y %H"), format = "%d/%m/%Y"), 
            tz = "UTC")
        surfDatDaily <- aggregate(surfT, by = list(day), FUN = mean, na.rm = TRUE)
        surfDatDaily <- surfDatDaily$x
    } else
    {
        surfDatDaily <- surfT
    }
...
getValDataDaily <- function(imisVar, startDate, endDate, station, file, daily, fun)
{
    ...
    imisDat <- get.ncdf.data(stationNr = station, varName = imisVar, startTime = startDate, 
        endTime = endDate, fileName = file)
    
    # imisDat<-aggregate.ts(imisDat$data, nfrequency=(1/6), FUN=mean)
    if (daily == T)
    {
        day <- as.POSIXct(strptime(format(imisDat$time, format = "%Y/%m/%d %H"), 
            format = "%Y/%m/%d"), tz = "UTC")
...
eraAirTSurfPreprocess <- function(nbox, map = NULL, k1 = 1, k2 = 1460, step, stationHeight, 
    boxHeight, lapse, Tsfile, origin, interpolate, daily)
    {
    # uses pressure level data for simplicity (i dataset needed) but extrapolates
    # from
    source("~/data/imis/src/imis_functions.r")
    source("~/experiments/bigsim/src/valTools.r")
    source("~/experiments/bigsim/src/getGridMeta.r")
    ...
    if (daily == T)
    {
        t_vec <- aggregate.ts(t_vec, nfrequency = nfrequency, FUN = mean)
    }
...
# ERA grid box returns tair at stationHeight (a) interpolated to 3hr step or (b)
# aggregated to daily NEED TO MAKE ACCEPT NPRESSURE LEVELS ARGUMENT
eraRhPreprocess <- function(nbox, map = NULL, var, lm, k1 = 1, k2 = 1460, stationHeight, 
    Tfile, Gfile, origin, interpolate, daily)
    {
...
eraRhSurfPreprocess <- function(nbox, map = NULL, var, isncdf, k1 = 1, k2 = 1460, 
    step, stationHeight, boxHeight, lapse, Tsfile, origin, interpolate, daily)
    {
    # uses pressure level data for simplicity (i dataset needed) but extrapolates
    # from
    source("~/data/imis/src/imis_functions.r")
...
eraSWin <- function(nbox, map = NULL, var, k1 = 1, k2 = 1460, step, Tsfile, origin, 
    interpolate, daily)
    {
    # uses pressure level data for simplicity (i dataset needed) but extrapolates
    # from
