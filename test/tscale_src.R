...
# ==============================================Description=====================================================
# PURPOSE = aggregates time variable to daily values, used as input to aggregate
# data IN = ncdf file OUT = daily ts of time variable DEPENDS ON = none
# ==============================================Parameters======================================================
# SETUP PARAMETERS:

## ERA interim fileName='/home/joel/data/era/pressureLevels/tair/outfile.nc'
## origin=1900 step='seconds' timeVar='time'

## METEOSWISS fileName='~/data/meteoswiss/meteoch.nc' origin=1970 step='hours'
## timeVar='time'

## IMIS fileName='~/data/imis/data/imis_data.nc' origin=1970 step='hours'
## timeVar='time'

# ASRB

# =============================================Function========================================================
timeDaily <- function(fileName = "/home/joel/data/era/pressureLevels/tair/outfile.nc", 
    origin = 1900, step = "seconds", timeVar = "time")
    {
    require(ncdf4)
    nc <- nc_open(fileName)
    ...
    dayTS <- as.POSIXct(strptime(format(time, format = "%Y/%m/%d %H"), format = "%Y/%m/%d"), 
        tz = "UTC")  # tz correct?
...
# ==============================================Description=====================================================
# PURPOSE = aggregates data to daily timeseries IN = dayTS OUT = daily ts of data
# DEPENDS ON = timeDaily
# ==============================================Parameters======================================================
# SETUP PARAMETERS:

## ERA interim fileName='/home/joel/data/era/pressureLevels/tair/outfile.nc'
## METEOSWISS fileName='~/data/meteoswiss/meteoch.nc' IMIS
## fileName='~/data/imis/data/imis_data.nc' ASRB

# GENERAL dat= vector variable timeseries to be aggregated dayTS= daily time,
# timeseries from timeDaily FUN= mean(default)/max/min
# =============================================Function========================================================
...
# ==============================================Description=====================================================
# PURPOSE = aggregates data in multidimensional form to daily timeseries in
# multidim form IN = dayTS OUT = daily ts of data DEPENDS ON = dataDaily
# ==============================================Parameters======================================================
# SETUP PARAMETERS:
...
# GENERAL dat= multidim variable timeseries to be aggregated MARGIN = dimensions
# to remain constant (ie not aggregated) dayTS= daily time, timeseries from
# timeDaily FUN= dataDaily

## EXAMPPLE

# str(dat) num [1:7, 1:4, 1:6, 1:20456]

## we aggregate dim 4, time, to daily values

## so z=apply(X=dat, MARGIN=c(1,2,3), FUN=dataDaily, dayTS=dayTS)

## gives a very strange data structure lis of z matrix of y cols and x rows:
## z[row,col,matrix] z[1:7, 1:4, 1:6]

## experiment to check dimensions transposed correctly ie dat=z col=7 row=1 pl=3
## dat1=dat[col,row,pl,] dat1agg=dataDaily(dat=dat1, dayTS=dayTS) str(dat1agg)
##'data.frame':\t5114 obs. of  2 variables:
## $ Group.1: POSIXct, format: '1996-01-01' '1996-01-02' ...  $ x : num 250 250
## 248 251 251 ... c=z[col,row,pl] # col, row, pl v=as.data.frame(c) str(v)
##'data.frame':\t5114 obs. of  2 variables:
## $ Group.1: POSIXct, format: '1996-01-01' '1996-01-02' ...  $ x : num 250 250
## 248 251 251 ... cor(dat1agg$x, v$x) [1] 1
...
# ==============================================Description=====================================================
# PURPOSE = extracts single time series from z IN = z, multidim aggregated series
# OUT = single dim aggregated timeseries (not ts object, is dataframe) DEPENDS ON
# = dataDaily
# ==============================================Parameters======================================================
# z=multidim aggregated series dims= vector corresponding to dims to select
# =============================================Function========================================================
...
# ==============================================Description=====================================================
# PURPOSE = cuts data to given start end dates IN = (1) dates vector, (2) data
# vector OUT = cut dates/data dataframe DEPENDS ON = NULL
# ==============================================Parameters======================================================

# dates=dat$Group.1 # dates vector, any class? data=dat$x # data vector
...
# ============================================Description======================================================
# PURPOSE = vector of ERA rid ele based on geopotential file IN = geop.nc OUT =
# DEPENDS ON =
# =============================================Parameters======================================================
# SETUP PARAMETERS:

# =============================================Function========================================================
# geop='/home/joel/data/era/surface/geop.nc' const=9.80665 require(ncdf)
# nc<-nc_open(geop) z = ncvar_get( nc, timeVar)
# =============================================================================================================

############################################################################################################### FUNCTION: d2toRhum #

# ============================================Description======================================================
# PURPOSE = convert ERA interim surface dewpoint temp to relative humidity IN =
# d2m and t2m OUT = rhum DEPENDS ON =
# =============================================Parameters======================================================
# SETUP PARAMETERS:
...
relHumCalc <- function(tair, tdew)
{
    ...
    lv <- 2500000  #latent heat of vaporization of water (2.5 * 10^6 joules per kilogram)
    Rv <- 461.5  #gas constant for water vapor (461.5 joules* Kelvin / kilogram)
    
    # variables tdew=dewpoint temp (kelvin) tair=air temperature (kelvin)
...
getGridBoxID <- function(eraFile = "/home/joel/data/era/surface/grid74/tair.nc", 
    metaFile = "/home/joel/data/transformedData/MET_meta.txt")
    {
...
################################################################################################################ ERA accumulated -> instant values only for lwin currently

accumToInstERA <- function(nbox, coordMap, file, origin, startDate, endDate, var, 
    step)
    {
    ...
    for (fld_name in fld_names)
    {
        ...
        if (fld_name == "ssrd.cell")
        {
            fld <- "ssrd"
            a <- ssrd.cell
        }
        if (fld_name == "strd.cell")
        {
            fld <- "strd"
            a <- strd.cell
        }
        ...
        for (n in 1:(tot_steps/2))
        {
            stepID <- n * step
            fc_start <- 0
            assign(paste(fld, "_s", stepID, "_", fc_start, sep = ""), a[seq(n, length(a), 
                8)])
        }
    ...
    # strd_s12_12=c(strd_s12_12,strd[length(strd_s12_12)]) duplicate last value as
    # missing strd_s9_12=c(strd_s9_12,strd[length(strd_s9_12)])
    # strd_s6_12=c(strd_s6_12,strd[length(strd_s6_12)])
    
    strd_df <- data.frame(strd_s3_0, strd_s6_0, strd_s9_0, strd_s12_0, strd_s3_12, 
        strd_s6_12, strd_s9_12, strd_s12_12)
    # ==========================================================
    ...
    for (n in 2:(tot_steps/2))
    {
        assign(paste("inst_val_", n, sep = ""), ((var_df[, n] - var_df[, n - 1])/step_time))
    }
    
    inst_val_5 <- var_df[, 5]/step_time  # first from forecast start
    for (n in 6:tot_steps)
    {
        assign(paste("inst_val_", n, sep = ""), ((var_df[, n] - var_df[, n - 1])/step_time))
    }
    
    strd_inst_val <- as.vector(rbind(inst_val_1, inst_val_2, inst_val_3, inst_val_4, 
        inst_val_5, inst_val_6, inst_val_7, inst_val_8))
...
# extractVals=function(shp, dem){ dem='/home/joel/data/dem_CH/aster30_ch1903.asc'
# shpFile='/home/joel/data/asrb/asrb_loc [Swiss. Obl.shp' proj='+proj=somerc
# +lat_0=46.95240555555556 +lon_0=7.439583333333333 +k_0=1 +x_0=600000
# +y_0=200000 +ellps=bessel +towgs84=674.374,15.056,405.346,0,0,0,0 +units=m
# +no_defs' rst=raster(dem) projection(rst)<-proj shp=shapefile(shpFile)
# slp=terrain(rst, opt='slope',unit='degrees') asp=terrain(rst, opt='aspect',
# unit='degrees')

# slpData=extract(slp, shp) return(slpData) }
...
# extract(asp, shp) return(asp) }



# } calc wind direction from vectors function calc wind speed [liston elder 2006]
# windDir<-function(u,v){ wd=(((3*pi)/2)- atan (v/u)) wd=wd*(180/pi) ws<-180/pi*s
# return(wd) } reduced range error 200--360 due to this liston elder 2006
# formula.
...
# source:
# http://stackoverflow.com/questions/8673137/calculating-wind-direction-from-u-and-v-components-of-the-wind-using-lapply-or-i

################################################################################################################ calc wind speed from vectors function calc wind speed [liston elder 2006]
...
windDirGrid <- function(nbox, map, varu, varv, k1, k2, fileU, fileV, daily)
{
    ...
    if (is.null(map) == T)
    {
        # setup for my weird 15 box grid (change to old automated code for sequence when
        # update grid)
    ...
    lat <- ncvar_get(u, "lat")
    ### suppose we have a time in seconds since 1960-01-01 00:00:00 GMT (the origin
    ### used by SAS)
    q <- time * 60 * 60  #make seconds
    ## ways to convert this
    date <- as.POSIXct(q, origin = "1900-01-01")  # local
    day <- as.POSIXct(strptime(format(date, format = "%Y/%m/%d %H"), format = "%Y/%m/%d"), 
        tz = "UTC")  # for aggegation
    dayvec <- day[k1:k2]
    # function calc wind speed [liston elder 2006] windDir<-function(u,v){
    # wd=(((3*pi)/2)- atan (v/u)) wd=wd*(180/pi) ws<-180/pi*s return(wd) }
...
# returns 6 hourly wind speed (m/s) for grid box
windSpdGrid <- function(nbox, map, varu, varv, k1, k2, fileU, fileV, interpolate, 
    nfreq, daily)
    {
...
################################################################################################################ Calculate Grid level wind speed and direction - needs to be returns 6 hourly
################################################################################################################ wind speed (m/s) for grid box
...
################################################################################################################ ERA accumulated -> instant values ONLY accepts accumulated values vector and
################################################################################################################ date vector.
...
accumToInstERA_simple <- function(inDat, step)
{
    ...
    for (n in 1:(tot_steps/2))
    {
        stepID <- n * step
        fc_start <- 0
        assign(paste(fld, "_s", stepID, "_", fc_start, sep = ""), inDat[seq(n, length(inDat), 
            8)])
    }
    ...
    var_df <- data.frame(strd_s3_0, strd_s6_0, strd_s9_0, strd_s12_0, strd_s3_12, 
        strd_s6_12, strd_s9_12, strd_s12_12)
    
    # basic formula ( FLD( STEP2)- FLD( STEP1))/(( STEP2-STEP1)*3600).
...
# =====================================================================================================================
# accepts vector interpolates between existing datapoints ie returns vector of
# (2*original vector -1) WARNING: final 21UTC missing in some cases if 6h data eg
# 01011996 00UTC -> 31122008 18UTC (should download +1 day in future to allow
# final 21UTC interpolation point)
interp6to3 <- function(dat6)
{
    ...
    dat_sorted <- df[ndx, ]
    # lastvalue=dat_sorted$data[length(dat_sorted$data)] t_vec3=c(dat_sorted$data,
    # lastvalue)
...
# =====================================================================================================================
# accepts vector accum vals are converted to step averages - but need to shift
# back to original timepoint. interpolates between existing datapoints ie returns
# vector of (original vector -1) shift 01.30UTC, 04.30UTC, 07.30UTC,... -> 3UTC,
# 6UTC, 9UTC.... WARNING: LOSES LAST DATAPOINT DUE TO INTERPOLATION between
# points
...
getNameVec <- function(mf1)
{
    ...
    mf <- read.table(metaFile, sep = ",", header = T)
    eraBoxEle <- read.table("/home/joel/data/transformedData/eraBoxEle.txt", header = T, 
        sep = ",")
    eraCells <- 28
    ...
    for (nbox in nboxSeq)
    {
        ...
        if (length(na.omit(singleGridboxPoints_vec)) == 0)
        {
            next
        }
...
Rh2Wvp <- function(tair, RH)
{
    ...
    # variables RH=relative humidity (%) tair=air temperature (kelvin)
    es <- es0 * exp(lv/Rv * (1/T0 - 1/tair))
...
dayDates <- function(fileName = "/home/joel/data/era/pressureLevels/tair/outfile.nc", 
    origin = 1900, step = "seconds", timeVar = "time")
    {
    require(ncdf)
    nc <- nc_open(fileName)
...
# output months
monthDates <- function(fileName = "/home/joel/data/era/pressureLevels/tair/outfile.nc", 
    origin = 1900, step = "seconds", timeVar = "time")
    {
    require(ncdf)
    nc <- nc_open(fileName)
...
# output years
yearDates <- function(fileName = "/home/joel/data/era/pressureLevels/tair/outfile.nc", 
    origin = 1900, step = "seconds", timeVar = "time")
    {
    require(ncdf)
    nc <- nc_open(fileName)
...
{
    sum(sim - obs, na.rm = T)/length(obs)
}
...
# 10/5/2013 DESCRIPTION: using ERA pressure level data, interpolates temperature
# profile to provide AirT timeseries for any given point in ERA grid cell domain
# accepts 2d matrix of data by time*pressurelevels IN: 1. ERA air temperature
# pressure level data (K) 2. ERA geopotential file (can be the same file) [m^2
# s^-2] 3. height (m asl) of interpolation point returns tair at stationHeight
...
plevel2point <- function(dat, gdat, stationEle)
{
    ...
    # vectorise d=apply(X=dat, MARGIN=c(1,), FUN=interp6to3, x=gph,
    # y=dat,xout=stationEle)
    
    
    return(t_vec)
}

# changed data structure (z,T reversed order due to new preprocessing of era
# data)
